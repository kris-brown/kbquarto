...
- functions $V \xrightarrow{in, out} \mathbb{N}$ give the \emph{in degree} and \emph{out degree} of each vertex
- A bijection $\bar m \sqcup O \xrightarrow{i} I \sqcup \bar n$, where $I = \{(v,i)\ |\ v \in V, 1 \leq i \leq in(v)\}$ and $O=\{(v,i)\ |\ v \in V, 1 \leq i \leq out(v))\}$ are the \emph{vertex inputs} and \emph{vertex outputs}, respectively.
- Furthermore, an \emph{acyclicity condition}:
  - Use the bijection $i$ to construct the \emph{internal flow graph}: a graph with an arrow $u \xrightarrow{e^{u,i}_{v,j}} v$ for every $i,j \in \mathbb{N}$ such that $i(u,i)=(v,j)$
...
- $V \sqcup V' \xrightarrow{[in,in']} \mathbb{N}$ maps elements of $V$ according to $in$ and elements of $V'$ according to $in'$ (likewise for \emph{out}).

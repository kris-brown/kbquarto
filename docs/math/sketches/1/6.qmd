...
::: {.callout-note collapse=true appearance="minimal"}
## Solution
1. Let the preorder be given by this diagram (with implicit reflexive arrows): \begin{tikzcd}2 \arrow[rr] && 3 \\& 1 \arrow[lu] \arrow[ru] &  \end{tikzcd}
2. Let $Q$ be given by the following diagram
    - \begin{tikzcd}2 && \looparrowright 3 \\& \looparrowright 1 \arrow[lu] &\end{tikzcd}
    - and let $Q'=S\times S$

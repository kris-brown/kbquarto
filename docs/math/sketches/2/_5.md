...
<div id="quantale"/></div>
-  A symmetric monoidal closed preorder $\mathcal{V}=(V,\leq,I,\otimes,\multimap)$ that has all [joins](../1/5.qmd#join).
...
Let $(P,\leq)$ be a preorder. It has all [joins](../1/5.qmd#join) iff it has all [meets](../1/5.qmd#meet).
...
Suppose $\mathcal{V}=(V,\leq,I,\otimes)$ is a [symmetric monoidal preorder](2.qmd#smp) that has all [joins](../1/5.qmd#join).
...
2. What is the [join](../1/5.qmd#join) $x \vee y$ for **Bool** and **Cost**?
...
::: {.callout-note collapse=true appearance="minimal"}
## Solution
The [joins](../1/5.qmd#join) all exist:

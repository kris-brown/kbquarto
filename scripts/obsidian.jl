...
function replace_link(title_dict, path_dict)
  function f(mtch)
    res = split(mtch[3:end-2],"|")
    ttl, disp = length(res) == 1 ? repeat(res,2) : res
    ttlp = split(ttl,"#")
    ttl, p = length(ttlp) == 1 ? (ttl,"") : (ttlp[1],"#"*ttlp[2])
    fixed_title = fix_title(ttl)
    return "[$disp](/$(joinpath(OBSID,path_dict[fixed_title], title_dict[fixed_title])).html$p)"
  end 
end

function process(pthfi, links_in, title_dict, path_dict)
  fi = last(split(pthfi,"/"))
  path_fi = joinpath(path_dict[fi], fi)
  real_name = joinpath(dirname(pthfi), title_dict[fi]) * ".md"
  println("processing $path_fi ($real_name)\n\n")
  txt = read(real_name, String)
  if !isempty(links_in)
    # Add the backlinks file to backlinks folder
    render_links(fi, links_in, path_dict)
    # Add the path to backlinks file in the metadata 
    t1,t2,t3... = split(txt,"---\n")
    n = count(==('/'), joinpath(OBSID,path_fi))
    txt = join([t1,t2*show_listing(n,fi),t3...],"---\n")
  end 
  txt = replace(txt,r"!\[\[([^\]]+)\]\]" => include_link(path_dict))
  txt = replace(txt,r"\[\[([^\]]+)\]\]" => replace_link(title_dict, path_dict))
  txt = replace(txt, "```mermaid"=>"```{mermaid}")
  newfi = joinpath(QUART, OBSID, path_dict[fi], fi*".qmd")
  mkpath(dirname(newfi))
  open(newfi, "w") do io
    write(io, txt)
  end
end
function include_link(path_dict)
  function f(mtch)
    fixed_title = fix_title(mtch[4:end-2])
    println("path_dict[fixed_title] $(path_dict[fixed_title])")
    return "{{< include \"/$(joinpath(OBSID,path_dict[fixed_title], fixed_title)).qmd\" >}}"
  end
end 

"""create a backlinks yaml file"""
function render_links(fi, links_in, path_dict)
  kpth = joinpath(BACK,"$fi.yml")
  mkpath(dirname(kpth))
  open(kpth, "w") do io
    for (link, ctx) in links_in 
      println(io,"- name: $(last(split(link,"/")))")
      println(io,"  href: /$(joinpath(OBSID,path_dict[link],link)).html")
...
# MAIN # 
########
title_dict, links_in, links_out, path_dict = get_links()
for (root, _, files) in walkdir(VAULT) 
  for file in filter(is_md, files)
    fi = fix_title(file[1:end-3])
    l_in = links_in[fi]
    process(joinpath(root,fi), l_in, title_dict, path_dict)
  end 
end 